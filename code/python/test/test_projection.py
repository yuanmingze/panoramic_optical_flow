...
def test_cubemap_flow():
    ...
    # 5) test stitch the cubemap images. Note enable test 2
    erp_image_src = proj_cm.cubemap2erp_image(face_images_src)
    image_io.image_show(erp_image_src)
    image_io.image_save(erp_image_src, "../../data/tar.jpg")

    # # 6) test stitch the cubemap flow. Note enable test 3
    # erp_flow_stitch = cubemap2erp_flow(face_flows)
    # erp_flow_stitch_name = cubemap_flow_output + "cubemap_stitch_flo_test.jpg".format(index)
