...
def depthmap2pointcloud_erp(depth_map, rgb_image, output_ply_file_path):
    ...
    :param rgb_image: The ERP rgb image, shape is [height, widht, 3], if None do not get color of point cloud.
    :type rgb_image: numpy
    :param output_ply_file_path: 3D point cloud output path.
    :type output_ply_file_path: str
    """
    # spherical coordinate
    pixel_x, pixel_y = np.meshgrid(range(depth_map.shape[1]), range(depth_map.shape[0]))
