...
The 360 Replica dataset rendering with two kinds of camera path circle and line. The circle path is the camera face to outward and moves along a circle (\TODO{how much radian and image per circle}), the line path is the camera face the same direction and moves in a straight line.
...
\begin{figure*}[hbt!]
	...
	\caption{\label{fig:exp:compflow}
		The optical flow SEPE heatmap on Replica 360 dataset, 1st and 2nd rows are the result on the Replica $office\_0$ result, which are optical flow and optical flow SEPE error heatmap respectively, darker is more error. And 3rd and 4th rows are part of the result on the Replica $apartment\_0$.}
\end{figure*}
