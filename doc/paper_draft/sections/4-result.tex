...
The 360° Replica dataset rendering with two kinds of camera path circle and line. The circle path ($Circle$) is the camera face to outward and moves along a circle (\TODO{how much radian and image per circle}), the line path ($Line$) is the camera face the same direction and moves in a straight line.
...
\begin{figure*}[hbt!]
	...
	\caption{\label{fig:exp:compflow}
		The optical flow SEPE heatmap on Replica 360° dataset, 1st and 2nd rows are the result on the Replica 360° $office\_0$ result, which are optical flow and optical flow SEPE error heatmap respectively, darker is more error. And 3rd and 4th rows are part of the result on the Replica 360° $apartment\_0$.}
\end{figure*}
...
\end{tabular}
	\caption{\label{fig:exp:backwardwarp}
		The optical flow backward warp result on Omniphoto dataset's $BeihaiPark$. The 3rd row is the image error heatmap, more error is darker.}
\end{figure}
...
%%{analysis result}
The `w/o ERP' error is higher than the `w/o cubemap' error for both `\emph{Line}' and `\emph{All}' data, because the ERP optical flow is the global motion, which can handle large pixel motion.
\CR{The last half of the sentence is not clear, please clarify.}
...
The result without blending weight is shown as `w/o weight' \CR{w/o blending?} in \cref{fig:exp:ablations}, which replaces all weights with the unit weight.
%{analysis result}
...
%	\setlength{\tabcolsep}{3pt}%
	\begin{tabular}{p{0.2cm}p{1.9cm}p{1.1cm}p{1.1cm}p{1.1cm}p{1.3cm}p{1.3cm}p{1.3cm}}
		...
		& w/o weight   & 4.854 & 0.1863 & 37.10 & \bf 0.007118 & \bf 0.05074 &     0.01899 \\
		...
		& w/o weight   & 10.13 & 0.2447 & 48.60 &     0.01600 & \bf 0.08798 &     0.03566 \\ 
		& w/o ERP      & 10.82 & 0.2648 & 56.48 &     0.01580 &     0.09090 &     0.03519 \\ 
		...
		& w/o weight   & 7.524 & 0.2130 & 42.94 &     0.01161 & \bf 0.06967 &     0.02741 \\
