...
\MZY{The tangent image optical flow just traces the pixel on the tangent image pair from the same ERP image area. This makes it hard to estimate the pixel moves out of the tangent image boundary.}
...
These images are perspective images with the same camera centre as the ERP image, but their optical axis\MZY{principle axis} intersects the unit sphere at the tangent point with a circumscribed cube or icosahedron.
%
...
%
\CR{Should the `$- \mathbf{x}$' be outside the parenthesis?}
...
%				= \mathcal{P}^{-1} \left( \hat{R}^{-1} \cdot \bar{R}^{-1} \cdot \mathcal{P}(\tilde{\mathcal{F}}^{EP}_{(i,j)}) - (i,j)\right)
	...
	= \mathcal{P}^{-1} \left( \hat{R}^\top \cdot \bar{R}^\top \cdot \mathcal{P}(\tilde{\mathcal{F}}^\text{EP}(\mathbf{x})) - \mathbf{x}\right)
	\text{.}
%\end{split}
\end{equation}
