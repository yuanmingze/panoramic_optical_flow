...
\new{We compute the global rotation $R$ \CR{should this be $\bar{R}$ to be consistent with \cref{fig:approach:pipeline,equ:approach:globalwarp}?}from the computed optical flow field as follows.
...
After \new{estimating} the global rotation $R$ from the 360° optical flow, we warp the target image $I_{t+1}$ by the rotation $R$ to align it to the source image $I_{t}$. %~\cite{ZioulKZAD2019}.
%
...
\ \\\TODO{Make more mathematical below}
